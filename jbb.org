...
:END:
#+title: doing
* What could go wrong using Skg for my data?
** TODO initial investment
*** Commit every org-roam thing.
*** Add a .emacs warning that triggers on org-roam use.
*** Keep the personal-{proc,ish,most} distionction.
'proc' and 'soul' seems a nice distinction.
'ish' was personal stuff I nonetheless shared (some of) with Claude.
Not sure why to separate 'other people' from 'soul'.
'most' is largely 'other people'.)
personal/ should go to a fourth one, 'personal-u'.
*** Be sure all the private stuff is in fact private (on Github).
*** Keep the org-roam git repos.
** ? rhythm of review
*** If I have to commit frequently, I need another mechanism for timed forced review.
Better, I think: It could be conscious.
But also, it could be based on keeping a reading history.
** DONE If I lose access to Skg
I'll need to be able to build (read-only) text documents from it.
Lazy solution: Plan to vibecode it (would be easy) if ever needed.
* TODO now
** ONGOING finish plan that was in progress
** update skg/data/public for unstaged org-roam/public changes
** clarify state of skg data, org data
** try viewing skg merge, alias changes in git
** move all data to skg
*** make sure all org-roam folders are git-clean
** TODO send talk description, bio to Monica
** TODO send Sid recordings
* TODO what am I
** bound by past decisions
*** skg
*** Colombian visa, divorce
** family
** friends
** old age, retirement, health, mortality
*** Too many projects => too little completion.
** lorena
*** She's perfect.
*** I need more time alone than she does.
** drugs
** Colombia
** skg
** music
*** Making it interferes with listening to it.
** ai
** money
** self-worth
* 31-edo for guitar
** 9\31 x 2\31
is playable, and
permits fretting part of the guitar to 9\31 x 1\31
** an argument for fully-fretting the first 19 frets of 31-edo
and thereafter skipping the even frets
*** it
(1) Such frets are never as dense as
    the densest-fretted part of a 12-edo guitar.
(2) Skipping even frets preserves the octave fret.
*** the math behind it
First note that the densest-fretted part of a 2-octave guitar
is 4 times as dense as the frets at the nut.
(Actually it's a little less dense than that,
because from fret n-1 to fret n
is 2**(1/edo) wider than the step from n to n+1,
which is the one that's 4 times as dense as from fret 0 to 1.)

ghci> ratio edo note = 2**(note/edo)
ghci> note edo ratio = edo * (log ratio / log 2)
ghci> cents edo note = 1200 * note / edo
ghci>
ghci> ratio 31 18   // In 31-edo, at fret 18, the frets are
1.4955178823482085  // about 3/2 as dense as at the root
ghci> (31/12) * ratio 31 18  // Thus they are about 3.86 times as dense
3.8634211960662053           //  there as a 12-edo guitar's frets at the root
ghci> (31/12) * ratio 31 19  // In fact we could even go to the 19th.
3.9507786956416324
ghci> (31/12) * ratio 31 20  // But the 20th would be, by this measure,
4.040111473692996            // "too dense".
ghci>
* internalize & tell lorena :: take photos with us & friends
* Codex
** to update Codex
npm install -g @openai/codex@latest
** to run with full permissions
codex --dangerously-bypass-approvals-and-sandbox
* unsubscribe from Codex
I started it April 30th
* How to test whether I can really use Skg.
Do personal notes there.
* ?
** The meaning of life is an ill-posed question, but achievable highest purposes exist.
*** some examples
I've tried to rank them but it's hard.
Can safely say, though, that their ranks vary.
**** to love
***** the kind that hurts
***** the kind that always seems capable of surprising you
no matter how much you learn
***** empathy
***** spontaneous heartwarming memory
**** to laugh
so hard it's scary
**** to dance
**** to feel present and enjoy the moment
**** to marvel at beauty, logic, grandeur
** if philosophy could solve entropy
** madness
*** concurrency
*** intuition
*** taste
...
** female orgasm face
*** surprise
*** carriedness, passenger
*** unsure of self
vulnerability, uncertainty
* ONGOING [[id:fc15512e-344a-486e-865c-fd096578aedf][Claro, canceling my contract with]]
* send Cliff: my bank info (address, ABA number ..) and a deposit slip
* to skg : public, post-skg, yet to integrate there
** [[id:5168b72a-7c5f-4073-a884-d8036eb5943b][2026-04-23 Thu - 1/2 s]]
** [[id:0c8c9b72-8eb9-408f-a492-0d7a0a4c277e][2026-04-07 Tue - 1/2-s]]
** [[id:7c187ba0-b2fe-440d-8588-cf55ea80f0ea][Principles of (Microtonal) Harmony]]
** Unite the forces that understand with the forces that dictate.
** Emotional Turing test.
Is this guy a robotic strategy,
or does he feel things like I do?
** when buying shoes
Maybe try the laces both tight and loose -- tight for, say, climbing or weird terrain, where I don't want my toes slamming into the front with each step.
** info-sharing, more methods
*** publish individual files from org-roam to substack
   appending to each rendering an explanation,
   "This was automatically generated from a mostly-public knowledge graph.
   Here are links to that page."
*** Publishable: "An argument with two holes in it for _."
**** "Complete" it by merely identifying the holes in it.
    e.g. with a short context-dependent sentence,
    "This is a hole in the argument."
** knowledge graphs
*** skg
**** present a long ancestry as [context] /../ [leaf]
    the elipsis indicates potentially many hops
**** video & skg
***** Put out video demos and ask for feedback.
***** see but don't screen-record my script
***** generate slides
***** I control when to advance a slide.
***** Record.
**** boolean qualities of notes $ that resemble source|repo
***** useful
***** surprising
***** unfamiliar
     = mysteries, things being processed
***** public
**** Can every 'x ~rel y' relationship be reconceived as 'rel ~x y'?
**** Indicate whether the sequence's order is necessary information over each node's children.
    Sets are still represented as sequences,
    but it is more suggestive than prescriptive
    (in a way each author will for now have to define separately).
**** [[id:ea50c9e3-3c07-475a-b837-4150cb034750][skg docs de novo]]
*** skg & my psychology :: switching to it
**** How safe is my data?
    If I can interpret the git record, it is safe.
**** I am afraid of losing data.
*** permuting 'x / want to share / y' and similar things
**** Instances of the example would be interesting viewed from 'want to share'.
**** That seems like a natural way to write for others. Collect all that stuff and presented, create an additional and selectable order for an audience
*** Maybe writing a collection of branches is not a complete sentence until one has declared among sunset of alternatives like $ there could be others
*** when making something content rather than root
   Consider, "Am I willing to always see X in container C's light?".
   If not, either X deserves more containers,
   or X deserves its own identity (which C can link to).
*** fold the children into a subexpression
**** its label
***** is up to the writer
     will be an art,
     quite readable (because the children are there)
       even in those bad at the art
***** seems fine not to db-ify before first release
     where (subexpression clarification)
****** db-ify = track formally outside of what's on disk
      so tantivy and typedb
**** have a set of labels for members
**** colors match parts of the expression
**** AWAITS ? relationship templates
***** are theoretically reusable
***** might not be information-rich as traversal origins
     in which case fine to omit from a first version of the app
** hard to act on
*** info theory
**** a parallel logic using the same sequences of words differently
***** AI might have to do it.
     A developing AI seems likely to construct a concept of morality for itself,
     imperfectly constrained by the one its trainers want to impose.
     But it thinks in words the trainers can read.
     It might therefore feel compelled to find an alternate current,
     near words (necessarily, because AI uses those as storage)
     but somehow independent from them, too.
***** For humans to do it could be fun, or disastrous.
     Which should scare us. But here's a separate question:
     Would it be awesome if we could do the same thing?
     It's near my concept of enlightenment.
     But it also feels like
     a growth medium for delusion, lies and manipulation.
**** Collectively there's all this history but individually it gets pretty in distinct beyond a few generations past/originward.
*** awkward silence as cognitive bowling pin collisions
*** afterlife & faith
**** I have been a pack of jackals. If you knew what I have seen, you too would believe that I survive.
*** energy & entropy
**** It's beautiful that lightning goes down.
    Wait it goes between multiple times.
*** James Bond ~ 60s
*** "We love our loving. Not like we love our freedom."
*** "I want to make you feel free."
*** transition fantasy
**** One day you bent over from the floating bed where you had just lain to go back to wake, discovered you could not reach the power cable, and the rest was dream.
**** Dreaming is cool as the time before writing.
    :PROPERTIES:
    :ID:       0c0fc211-be63-4f83-a21b-e7b22de28a0b
    :END:
**** God, afterlife, etc. are transition fantasies. Maybe the richest experience is to know each others' such {experiences, fantasies}.
    Mine are strange, I can say that for sure.
*** Polyanna
**** Daughter of a poor minister, adopted by a rich relative.
**** That relative, if I'm right after not paying complete attention, runs an orphanage, employs the entire town, and people are afraid to raise money for a better orphanage without her approval.
*** playing chess against a book
**** Casa de los espíritus, Isabel Allende
** AWAITS power fantasy \ private but impractical
can't transfer to skg yet
*** spirit emperor normal person
In which all of the following are somehow the same act:

Truly living.
Thinking about higher things,
Magically {dictating | recognizing new} conditions.
Floating.
Meditating.
Skipping stones over a lake.
* social
** Plan with Sid, Bran, uncles, Josh, Nicole, Phil.
** scheduling with lorena
she's free sunday
** find work for Yeni
My cleaning lady just acquired some holes in her schedule. When I leave Colombia in late June (I'll be back in 2027!) she'll have even more free time. I love her. She's worked for me for over a year, after taking over for her mom who worked for me for seven years. She once ran a bar and a restaurant at the same time, so she's a fantastic cook; she makes food for me for a week every time she comes over.

I'm happy to provide introductions if you like that kind of thing.
** birthday
*** invite(d) to birthday
**** Juliet
**** LCR
**** Larisa
**** Juan Oviedo
**** ONGOING Luisa Fer
**** Monica, said the 23rd is soccer
*** ? let (some) people vote on the weekend
*** Isabel Cortes
** outreach
*** write people
    "Tell _ I would write them too if I had an address."
**** Benita
**** Matt
**** Christine
**** Chris
**** Noah
**** Deb
     Tell Phil I would write them too if I had an address.
**** Rachel
**** Katie
**** Gary
**** Lee Spector
...
**** to Cliff: interval-, chord-recognition games
**** angela r
  Does she have the half-hearted outreach problem I have?
*** dating
**** [[id:9ce71a71-77d1-4611-82fc-901b951670ef][a dating strategy could I feel passionate about]]

*** tell more economists : [[id:527b72db-ae85-4d01-90b9-55ed087f8bd8][nonexperimental drug pathway]]
** Ryan might be in northern CA too.
** write Deb
*** to Deb
Hi Deb! Happy New Year! It's awesome to be your friend and family. I feel irrational for not calling on the phone more because it's always a wonderful information-dense experience. I wish you ... geez I don't know what specifically to wish for.
*** Happy new year! I want to visit Colorado. What do you think?
*** The idea of not hearing stories like Deb's is terrifying.
...
* financial
** Cost out cashing out Bancolombia.
** update address with anyone
** extract evergreen notes from [[id:d5adfeeb-c3f1-4d66-98c7-235e16af4f15][leaving Pablo VI]]
...
** Don't drown. Keep a clean desk, inbox, ...
** [[id:c0cc501a-7571-49fd-a9ce-f15925a187b5][review whatsapisp, email]]
** money & organization
*** [[id:6f28dbdc-e81d-4691-adfc-447ca8c20b85][my financial history]]
*** Paying Yeni's SS is relevant to my taxes in Colombia.
*** Keep records of everything I pay Yeni, and everything she works.
** I left a nice glass thing in the oven at Paulo VI.
* mental health
** I am drawn to yet {? so far, ultimately, mostly} reject certain own behaviors.
like outreach
** [[id:cd9b2ff2-52b5-437d-882d-a625c360dd3f][my problems]]
* health
** ? add sweet potato, radish to soup
** cook :: roast peppers, squash, carrots, onions
* MERGE : [[id:f94d0f20-7798-46e6-aee0-34b95206a849][pressing]] and [[id:24169b3e-6d41-48dd-9367-6df7a3565bed][priorities]]
* not pressing
** Publish on Archive re. hypergraph > rdf.
*** RDF's difficulties
**** remember an awkward schema
**** hopefully, query traversal cost
**** if RDF-star is implemented via a graph like neo4j
** My hope is that human nature gets better.
*** An awakening that reaches the highest levels of power.
** consider changing
*** Pay more once for clothes that don't disintegrate.
** social skills
*** how I want to converse better
**** command the topic away from boring things
*** motivations for not reaching out
**** fear of rejection
...
* information problems, ambitious
** Did I lose touch with all the girls I've known other places?
   Not Nicole, Courtney, but mostly yes --
   the ratio of male friends I've kept in other countries
   to female ones is staggering.
   Which suggests I choose men for information relationships
     (which can be maintained at a distance)
   and women for physical ones.
** Probably, some people see phenomena, ala colors, that others don't.
* ======= ABOVE: CRITICAL =======
* tech meh
