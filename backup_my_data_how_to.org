...
* AWAITS [[id:2e44ac14-b823-44ee-849a-033b574ee38b][asking about changing passphrase in Borg]]
* AWAITS ? process improvements
...
* AWAITS borg pwd compromised to rclone again
...
*** du-home-for-borg.sh
    this writes the file [[id:ca2dfd85-b9eb-4d22-b56d-b522856e536f][memory used in home folder]]
*** see the diff in org-mode
    cd ~/org-roam/personal-proc
    git diff memory_used_in_home_folder.org | \
      python3 ~/bin/du-diff-to-org.py > changes-by-size.org
    # Then visit the file in Emacs, from this buffer:
    changes-by-size.org
**** the old way
     git diff memory_used_in_home_folder.org > python3 ~/hodal/skg/memory-diff/du-diff-to-org.py
...
** after nixos-rebuild : check native disk usage
   serial, with backtracking
*** df -h .
*** If free space is low, [[id:d014e570-e8da-4661-8111-01d19947e2b2][clean Nixos]], [[id:d623ee48-2989-4802-82f9-3d51ddf45c19][clean Docker]], [[id:2ad411df-0e62-423f-8a77-8c92618707d9][clean Rust]], record date.
    last done <2026-04-13 Mon>
...
** view, delete recent photos on phone
   Most recently done:
   <2026-02-10 Tue>, read and deleted back to Halloween 2025.
   <2025-01-20 Mon> was the last time recorded before then.
** review whatsapp, email
   :PROPERTIES:
   :ID:       c0cc501a-7571-49fd-a9ce-f15925a187b5
   :END:
   on (maybe <2025-11-11 Tue>)
   I read chat titles backward to 10/29 (not inclusive)
** read spam
   inc. Outlook, Mintranet
** Listing It
   :PROPERTIES:
   :ID:       634bf462-cf6a-4394-a09e-f4c973c6a240
   :END:
...
** git push org-roam
** AWAITS [[id:b69aad4a-1006-4415-9e1f-b0af93ff699e][modify export to keep no history]]
...
* AWAITS ? strip these from borg backups
...
* make backups
  serial
** AWAITS ? [[id:61f661d1-a518-4846-8452-358d17a01508][Clean Reaper data from borg repo.]]
** mount external drive
** parallel
*** find ~/ugh/enc \( -type f -o -type d -o -type l \) ! -type s | zip -@ -0 /run/media/jeff/ext_hdd.9g.tshb/enc,org-hdd.zip
*** find ~/ugh/enc \( -type f -o -type d -o -type l \) ! -type s | zip -@ -0 ~/enc,org-ssd.zip
...
*** AWAITS ? [[id:a723c225-dd11-4039-9224-a2dabc158022][exclude more files]] from borg HDD
...
* do at home, backup tasks to
  ...
  :END:
** [[id:e1eda15a-df86-4050-b150-e8034ae49019][rebuild NixOS]]
*** [[id:603208d3-968d-43f5-a083-fdda645037ab][when I last did it]]
...
* magit this file
* DONE ? could restore ~/Downloads
  accidentally deleted <2026-02-19 Thu>
